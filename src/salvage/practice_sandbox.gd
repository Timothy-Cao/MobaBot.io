	...
	match game.practice_page:
		...
		"Player":
			for i in range(5):
				...
				toggle.name=field
		"Enemies":
			var types: Array=["dummy","bumper","charger","tank","breacher","mender","scatter","emp","lancer","volley","bomber","rammer","artillery","foreman","mosquito"]
			PracticeView.select(ui,["Target dummy","Bumper","Charger","Tank","Breacher","Mender","Scattergun","EMP suppressor","Arc lancer","Burst battery","Bomb carrier","Rammer","Artillery","Boss","Mosquito"],types.find(game.practice_enemy),Rect2(26,173,278,33),func(i): game.practice_enemy=types[i])
