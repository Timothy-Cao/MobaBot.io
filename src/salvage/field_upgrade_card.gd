...
static func draw(ui, run, id: String, index: int) -> void:
	...
	var title: String="200 credits" if credit else "XP gain" if id=="xp_gain" else "Pickup range"
	ui._ability_icon(card,"field_credit" if credit else "magnet" if id=="pickup_range" else "reward_point",Rect2(88,20,48,48))
