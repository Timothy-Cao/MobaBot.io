		...
		while not records.eof_reached():
			...
			if not line.is_empty():
				...
				if record is Dictionary:
					last = record
	valid = valid and last.get("result", "") in ["won", "lost"] and last.get("build", "") in ["slice-01", "slice-02", "slice-03", "slice-04", "slice-05", "slice-06-demo", "slice-07-demo", "slice-08-demo"]
