...
func _initialize() -> void:
	...
	for chapter in [1,4,8]:
		for round_index in range(3):
			for tier in [0,1,3,5]:
				for focus in ["balanced","utility","looting","pet"]:
	...
	var a=Reference.build(1,0,1,"utility"); var b=Reference.build(1,0,1,"looting")
	check(OperationRules.boss_hp(a)==OperationRules.boss_hp(b),"Enemies do not adapt to the player's spending")
	check(Reference.measure(b).effective_health==Reference.measure(a).effective_health,"Mastery focuses do not grant raw defensive stats")
	var again=Reference.build(1,0,1,"utility")
