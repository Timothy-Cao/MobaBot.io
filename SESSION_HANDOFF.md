...
- Latest owner feedback says the player hull bar is too small and keeps normal Q simple/straight, explicitly withdrawing the briefly proposed guided hold-Q. Projectile-wall collision remains a future hypothesis. The latest module proposal replaces Bulwark with an energy-regeneration/detonation totem, makes Reserve states explicit with optional detonation, and replaces Overclock with a ten-second transforming turret form whose Q/W/E/R become siege actions; D/F are disabled in that form. Conveyor terrain is a related exploration. These are documented in `GAMEPLAY_VISION_18.md`; none is implemented in the frozen build.
