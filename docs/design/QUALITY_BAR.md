...
11 September 2026, owner direction: Vanguard is in a good place and should remain the current benchmark; the next gameplay-design work should refine another class. A separate research pass finds that strong stylized survivor quality is feasible with Codex, Godot, Blender and a focused month of Meshy, while a full 3D conversion remains a distinct production experiment rather than part of the class pass. Cost ranges, reference comparisons and a bounded visual spike are recorded in [AI_ASSISTED_PRODUCTION_FEASIBILITY_18.md](AI_ASSISTED_PRODUCTION_FEASIBILITY_18.md). This is research/owner feedback only and does not change the current build or numeric ratings.

11 September 2026, animation-economy follow-up: the owner asks for high-return techniques that create perceived animation/graphic quality without authoring exhaustive frames. The resulting plan prioritizes intentional pose extremes, impact timing, a canonical rig, procedural aim/lean/hover/recoil, unified materials, VFX/audio layering, modular bodies and an optional Blender-rendered-sprite bridge. It proposes strict per-actor motion budgets and a four-treatment moving-combat bakeoff; see [HIGH_ROI_VISUAL_PRODUCTION_18.md](HIGH_ROI_VISUAL_PRODUCTION_18.md). This remains research and does not authorize camera shake, hit-stop, asset replacement or a renderer migration.

11 September 2026, owner A/B clarification: the current cartoony presentation is already a valid, fun style and must not be removed or degraded by the fidelity research. The TODO is to build a separate higher-fidelity/high-graphics presentation of the same evolving core gameplay, then A/B test both with players. Treat the high-fidelity work as a parallel presentation pipeline, not a replacement plan; hold gameplay and test conditions equivalent enough that visual preference and readability evidence are interpretable.

10 September 2026, later owner correction and Vanguard-module direction: withdraw the guided hold-Q idea and keep normal Impact Bolt simple; the separate desire to restore projectile-wall collision remains. Replace Vanguard's candidate Bulwark with a fast-energy-regeneration totem, make Reserve's away-charge/inside-drain/empty states unmistakable, and allow either support to be reactivated as a sacrificial explosion. Replace Overclock's aura with a one-second-entry/half-second-exit planted turret form lasting at most ten seconds: passive gun off; Q rapid spread micro-missiles; W two-second long-range fire; E exclusive channeled repair; R a 30-second emergency local EMP. D and F are explicitly disabled in turret form. Conveyor terrain may move the otherwise planted form. Exact balance and interaction rules remain prototype questions in [GAMEPLAY_VISION_18.md](GAMEPLAY_VISION_18.md); none is implemented or scored.

10 September 2026, owner player-readability/Q/cover feedback: the player hull bar is too small and should become substantially more visible. The owner also proposes holding Q to extend Impact Bolt flight to roughly twice its duration with gradual mouse-direction guidance, particularly for distant targets and routing around walls, and leans toward restoring projectile-wall collision. Current implementation still uses the small world bar, straight Q and permeable shot cover. The organized proposal preserves tap Q, gives guidance an attention/action cost, and distinguishes discrete projectiles from ground effects/beams before any rule change. See [GAMEPLAY_VISION_18.md](GAMEPLAY_VISION_18.md). No implementation or score change is claimed.

10 September 2026, owner gameplay-vision feedback: difficult combat should prompt situational use of the full kit across repositioning, retreat, sustained damage, kiting and burst. No single skill, combo or item should universally dominate; tools should have contextual strengths, credible alternatives and meaningful costs. Player and enemy growth should remain perceptible within rounds and across stages, with controlled run variance and useful partial progress. Low-input builds belong primarily in cleared/lower content, while frontier pushing rewards active execution. Reserve's leave-and-return identity is favored; Bulwark's apparently free value and Overclock's aura are marked for reconsideration, with a committed rooted burst-stance concept retained as an unapproved prototype hypothesis. Later same-day ideas add rare monster hull-repair drops and readable vision-interference specialists, including a range-limited jammer and a distinct hacked-perception experiment. Healing must remain optional variance rather than required luck; vision pressure must preserve critical tells, counterplay and accessibility. Full wording and acceptance questions are in [GAMEPLAY_VISION_18.md](GAMEPLAY_VISION_18.md). This is owner direction, not implemented balance or evidence for changing numeric ratings.
...
## 12 September 2026 — bullet-heaven design research

Owner requested strong applicable lessons from successful survivors games, with early/overall progression central and aimed mechanical play preserved. BULLET_HEAVEN_LESSONS_38.md records developer sources, code findings at c1905f0 and proposed experiments. Highest priorities: first-combo access and focused early investment, encounter pressure/payoff with meaningful factory routes, then full-run pause/milestone/gear calibration. Pushback: exact enemy scaling with every player improvement can erase felt growth; older fodder should become easier while new problems challenge execution. No quality rating changed. No new human test or competitor hands-on session, and no gameplay edits. Targets require owner review and human calibration.
