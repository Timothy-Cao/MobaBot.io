...
The owner's next 9 September note parks the combo kit and narrows design attention to the summon-network character. Linked turrets may share target information while retaining individual hard firing ranges; a proposed core active fires toward the mouse from the player and all eligible summons. The complete Relay Marshal proposal in [RELAY_MARSHAL_DESIGN_17.md](RELAY_MARSHAL_DESIGN_17.md) is assistant-authored and unplayed. Preserve the idea without changing scores or treating its exact loadout as locked.
